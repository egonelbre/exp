...
func main() {
	...
	for _, item := range matches {
		...
		if unicode.IsDigit(rune(filename[0])) {
			varname = "Section_" + filename
		}

		varname = strings.Replace(varname, ".", "_", -1)
		varnames = append(varnames, varname)

		examples[varname] = example{filename, "", data}
	}

	fmt.Println(`package smime
...
func _base64(s string) []byte {
	...
	for _, varname := range varnames {
		...
		ex := examples[varname]
		fmt.Println("var " + varname + " = _base64(`")

		b64 := base64.StdEncoding.EncodeToString(ex.Data)
