...
type TwoHashShiftTable struct{}

func (TwoHashShiftTable) Generate(p Printer, keywords []string) {
				...
				next:
					for shift1 := byte(0); shift1 < 8; shift1++ {
						...
						for i, keyword := range keywords {
							h := twoHashCalc(keyword, comb0, comb1, shiftn, shift0, shift1)
							if seen[h] {
								continue next
							}
							hashes[i] = h
						...
						fnname := fmt.Sprintf("TwoHashTable_%s%s_Shift%d%d%d", comb0.Name, comb1.Name, shiftn, shift0, shift1)
						p.FuncName(fnname)
						p.F("func %s(name string) bool {\n", fnname)
						p.F("if len(name) < 2 { return false }\n")

						maxhash := hashes[len(hashes)-1]
						p.F("h := %s\n", twoHashFormat(comb0, comb1, shiftn, shift0, shift1))
