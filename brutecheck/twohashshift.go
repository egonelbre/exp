...
type TwoHashShift struct{}

func (TwoHashShift) Generate(p Printer, keywords []string) {
			...
			for shiftn := byte(0); shiftn < 8; shiftn++ {
				...
				next:
					for shift1 := byte(0); shift1 < 8; shift1++ {
						...
						fnname := fmt.Sprintf("TwoHash_%s%s_Shift%d%d%d", comb0.Name, comb1.Name, shiftn, shift0, shift1)
						p.FuncName(fnname)
