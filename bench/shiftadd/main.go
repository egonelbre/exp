...
func testVariantGamma() {
	for low := int64(2); low < 63; low++ {
		for high := low + 1; high < 63; high++ {
			constant := 1 | (int64(1)<<high - int64(1)<<low)

			if !isVariantGamma(constant) {
				fmt.Println("INVALID GENERATOR", constant)
				continue
			}
			...
			for _, num := range numbers {
				...
				part := uint64(constant - 1)
				got := multiplyVariantGamma(num,
					byte(bits.TrailingZeros64(part)),
					byte(63-bits.LeadingZeros64(part)+1),
				)
...
func multiplyVariantGamma(x int64, low, high byte) int64 {
	return (x + x<<high) - x<<low
}

// isVariantAlpha checks for c.
...
func isVariantAlpha(c int64) bool {
	part := uint64(c - 1)
	return c >= 1 && part&1 == 0 && bits.OnesCount64(part) == 2
}

// isVariantBeta checks for c, where
...
func isVariantBeta(in int64) bool {
	part := uint64(in - 1)
...
// isVariantGamma checks for c, where
// x * c; where c == 1 + 2^N - 2^M, N > M
func isVariantGamma(in int64) bool {
	part := uint64(in - 1)
	if in < 5 {
		return false
	}

	highbit := 63 - bits.LeadingZeros64(part) + 1
	lowbit := bits.TrailingZeros64(part)

	return in == 1 + 1<<highbit - 1<<lowbit
}
