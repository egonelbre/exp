...
TEXT ·ArmAxpyUnsafeInterleaveXR4(SB), NOSPLIT, $0-48
	FMOVS alpha+0(FP), F0
...
loop_unroll:
	FMOVS  (R0)(R6<<2), F1 // f1 = xs[xi]
	ADD  R6, R1, R6 // xi += incx
	FMOVS  (R0)(R6<<2), F2 // f1 = xs[xi]
	ADD  R6, R1, R6 // xi += incx
	FMOVS  (R0)(R6<<2), F3 // f1 = xs[xi]
	ADD  R6, R1, R6 // xi += incx
	FMOVS  (R0)(R6<<2), F4 // f1 = xs[xi]
	ADD  R6, R1, R6 // xi += incx

	FMOVS  (R2)(R7<<2), F5 // f2 = ys[yi]
	FMADDS F0, F5, F1, F5  // f1 = alpha * f1 + f2
	FMOVS  F5, (R2)(R7<<2) // ys[yi] = f1
	ADD  R7, R3, R7 // yi += incy

	FMOVS  (R2)(R7<<2), F5 // f2 = ys[yi]
	FMADDS F0, F5, F2, F5  // f1 = alpha * f1 + f2
	FMOVS  F5, (R2)(R7<<2) // ys[yi] = f1
	ADD  R7, R3, R7 // yi += incy

	FMOVS  (R2)(R7<<2), F5 // f2 = ys[yi]
	FMADDS F0, F5, F3, F5  // f1 = alpha * f1 + f2
	FMOVS  F5, (R2)(R7<<2) // ys[yi] = f1
	ADD  R7, R3, R7 // yi += incy

	FMOVS  (R2)(R7<<2), F5 // f2 = ys[yi]
	FMADDS F0, F5, F4, F5  // f1 = alpha * f1 + f2
	FMOVS  F5, (R2)(R7<<2) // ys[yi] = f1
	ADD  R7, R3, R7 // yi += incy
...
TEXT ·ArmAxpyPointerLoopXR4(SB), NOSPLIT, $0-48
	FMOVS alpha+0(FP), F0
...
loop_unroll:
	FMOVS  (R0), F1        // f1 = *xs
	FMOVS  (R2), F2        // f2 = *ys
	FMADDS F0, F2, F1, F2  // f1 = alpha * *xs + *ys
	FMOVS  F2, (R2)        // *ys = f1
	ADD    R1<<2, R0, R0   // xs += incx * 4
	ADD    R3<<2, R2, R2   // ys += incy * 4
